...
def poe(mus: List[torch.Tensor], stds: List[torch.Tensor]) -> D.Normal:
    """
    Product of Experts (PoE) for combining Gaussian distributions, where input is the standard deviation (std).

    Parameters:
        mus : list of torch.Tensor
            List of mean tensors for each Gaussian.
        stds : list of torch.Tensor
            List of standard deviation tensors for each Gaussian.
    ...
    Returns:
        D.Normal :
            ...
            - combined_std: torch.Tensor
                Standard deviation of the combined Gaussian distribution.
    """
    # Add prior distributions with zero mean and unit variance (standard deviation = 1)
    try:
        mus = [torch.zeros_like(mus[0])] + mus
    except:
        print(mus)

    stds = [torch.ones_like(stds[0])] + stds  # Standard deviation for the prior is 1

    # Calculate precision (1/std^2) and combined precision
    precisions = 1 / torch.stack(stds, dim=1) ** 2
    ...
    # Calculate the combined standard deviation (the precision is 1/std^2)
    combined_precision = precision_sum  # This is the combined precision
    combined_std = 1 / torch.sqrt(
        combined_precision)  # The standard deviation is the inverse of the square root of the precision

    # return combined_mean, combined_std
    return D.Normal(combined_mean, combined_std)
